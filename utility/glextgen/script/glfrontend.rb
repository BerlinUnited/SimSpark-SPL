...
module GLExtGen
	class GLFrontEnd
		...
		def addFile (fileName, extensionExp, functionExp, extensionHash)
			files.concat([[fileName, Regexp.new(extensionExp), Regexp.new(functionExp), extensionHash]])
		end
		...
		def scan (fileName, extensionExp, functionExp, extensionHash)
			...
			File.open(fileName).each { |line|
				...
				if md = extensionExp.match(line)
					...
					# if we don't knwo the extension, we have to add it
					if currentExtension == nil
						currentExtension = GLExtension.new(extensionName)
						extensionHash[extensionName] = currentExtension
					end
					# at this point currentExtension has a valid GLExtension object ... always!				
