...
{
  //actlog(100, SLC->network()) << "ReadBuffFD: forgetting fd " << fd << std::endl;
  fd = -1;
}
...
spades::ReadBuffFD::setNewFD(int new_fd)
{
  ...
  if (fd != -1)
	...
	std::cerr << *this << ": Error on fcntl " << errno << ": " << strerror(errno) << std::endl;
    }
}
...
spades::ReadBuffFD::close()
{
  ...
  if (::close (fd) != 0)
    {
      std::cerr << *this << ": Error on close " << errno << ": " << strerror(errno) << std::endl;
      ret = false;
    }
...
spades::ReadBuffFD::readline(char* buf, int maxlen, char delim)
{
  for (;;)
      ...
      for (int i=0; i<read_buffer_len; i++)
	{
	  if (*rb == delim)
	    {
	      //actlog(220, SLC->network()) << *this << ": satisfying from read buffer " << i << std::endl;
	      memcpy(buf, read_buffer, i+1);
	      if (i!=read_buffer_len)
	  ...
	  if (errno == EAGAIN)
	    {
	      ...
	      //actlog(220, SLC->network()) << *this << ": no more data " << read_buffer_len << std::endl;
	      return 0;
	    }
	  else if (errno == EINTR)
	    {
	      /* SMURF: Note that we really could keep trying in this case. However,
		 I want to avoid looping on a single socket. A select call on this
		 socket will still return immediately, so it should slow us down
		 (much). Plus I don't think we get interrupted all that often */
	      //actlog(200, SLC->network()) << *this << ": interrupted system call, returning" << std::endl;
	      return 0;
	    }
	  else
	    {
	      std::cerr << *this << ": error on read "
	...
	{
	  /* we got an eof! */
	  //actlog(220, SLC->network()) << *this << ": EOF with " << read_buffer_len << std::endl;
	  if (read_buffer_len)
	    {
	      /* let's copy whatever data we have */
	      int len = read_buffer_len;
      ...
      /* res is > 0 */
      //actlog(220, SLC->network()) << *this << ": read " << res << std::endl;
      read_buffer_len += res;
	  ...
	  if (errno == EAGAIN)
	    {
	      ...
	      //<< read_buffer_len << " available, < " << len << std::endl;
	      return 0;
	    }
	  else
	    {
	      std::cerr << *this << "::readFixedLength: error on read "
		       << errno << ": " << strerror(errno) << std::endl;
	      return -1;
	    }
	}
...
spades::ReadBuffFD::reallocReadBuffer(int new_len)
{
  ...
  if (new_len > 1 << 16)
    std::cerr << "ReadBuffFD::reallocReadBuffer: new_len seems really big: " << new_len << std::endl;
#endif	
