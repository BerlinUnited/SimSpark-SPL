...
void
SPLRule::MoveBall(const Vector3f& pos)
{
...
void
SPLRule::AutomaticSimpleReferee(TPlayMode playMode)
{
...
void
SPLRule::ResetFaultCounterPlayer(int unum, TTeamIndex idx)
{
...
void
SPLRule::ResetFaultCounter(TTeamIndex idx)
{
    for(int t=1; t<=11; t++) {
...
void
SPLRule::ProcessAgentState(salt::Vector3f pos, int unum, TTeamIndex idx)
{
...
void SPLRule::SimpleOrder(float dArr[][3], int oArr[][3], TTeamIndex idx)
{
...
void SPLRule::CalculateDistanceArrays(TTeamIndex idx)
{
...
void SPLRule::AnalyseTouchGroups(TTeamIndex idx)
{
    if (idx == TI_NONE || mBallState.get() == 0)
        return;
    SoccerBase::TAgentStateList agent_states;
...
void SPLRule::ResetTouchGroups(TTeamIndex idx)
{
    SoccerBase::TAgentStateList agent_states;
    if (! SoccerBase::GetAgentStates(*mBallState.get(), agent_states, idx))
        return;
...
void SPLRule::AnalyseFaults(TTeamIndex idx)
{
    TTeamIndex idx2;
    if (idx == TI_LEFT)
        idx2 = TI_RIGHT;
...
salt::Vector3f SPLRule::RepositionOutsidePos(salt::Vector3f posIni, int unum, TTeamIndex idx)
{
    salt::Vector3f pos;
...
void
SPLRule::ClearPlayersAutomatic(TTeamIndex idx)
{
    if (idx == TI_NONE || mBallState.get() == 0)
        return;
...
void
SPLRule::ClearPlayers(const salt::Vector3f& pos, float radius,
...
void
SPLRule::ClearPlayers(const salt::AABB2& box,
                               float min_dist, TTeamIndex idx)
{
    if (idx == TI_NONE || mBallState.get() == 0) return;
    SoccerBase::TAgentStateList agent_states;
...
void SPLRule::ClearPlayersBeforeKickOff(TTeamIndex idx)
{
    if (idx == TI_NONE || mBallState.get() == 0) return;
...
void
SPLRule::UpdateKickOff(TTeamIndex idx)
{
...
void
SPLRule::UpdateKickIn(TTeamIndex idx)
{
...
void
SPLRule::UpdateFreeKick(TTeamIndex idx)
{
    // do nothing for the duration of mKickInPauseTime
    if (mGameState->GetModeTime() < mKickInPauseTime)
    {
...
void
SPLRule::UpdateGoalKick(TTeamIndex idx)
{
    // do nothing for the duration of mKickInPauseTime
    if (mGameState->GetModeTime() < mKickInPauseTime)
    {
...
void
SPLRule::UpdateCornerKick(TTeamIndex idx)
{
    // do nothing for the duration of mKickInPauseTime
    if (mGameState->GetModeTime() < mKickInPauseTime)
    {
...
void
SPLRule::Update(float deltaTime)
{
    if (
        (mGameState.get() == 0) ||
        (mBallState.get() == 0) ||
...
void
SPLRule::Broadcast(const string& message, const Vector3f& pos,
...
void
SPLRule::UpdateOffside(TTeamIndex idx)
{
#if 0
    // do nothing for the duration of mKickInPauseTime
    if (mGameState->GetModeTime() < mKickInPauseTime)
...
void
SPLRule::ClearPlayersWithException(const salt::Vector3f& pos,
...
Vector2f
SPLRule::GetFieldSize() const
{
    return Vector2f(mFieldLength,mFieldWidth);
}

void
SPLRule::ResetAgentSelection()
{
    SoccerBase::TAgentStateList agent_states;
    if (SoccerBase::GetAgentStates(*mBallState.get(), agent_states, TI_NONE))
    {
