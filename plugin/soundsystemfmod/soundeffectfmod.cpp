...
void SoundEffectFMOD::Load(const char* inName)
{
	...
	mHandle = FSOUND_Sample_Load(FSOUND_FREE, inName, FSOUND_NORMAL, 0);
	SetFileName(inName);
}
...
void SoundEffectFMOD::Load(void *inBuffer, int inSize)
{
	...
	mHandle = FSOUND_Sample_Load(FSOUND_FREE, (const char*)inBuffer, (FSOUND_NORMAL | FSOUND_LOADMEMORY), inSize);
}
