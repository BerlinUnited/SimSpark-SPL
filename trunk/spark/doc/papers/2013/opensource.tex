...
traffic and therefore the latency will be reduced considerably.\review{this does not quite fit to the rest of section where you argue that the proxy exactly removes the need for such optimizations}
...
As a powerful robot simulator, SimSpark is gaining popularity in RoboCup, including Standard Platform League and Humanoid League.\review{may be this is too obvious, but the main application is still 3D simulation league that is not mentioned here. It should be. That has the most impact as of today.} It is also widely used to teach artificial intelligence and robotic lectures.
...
\review{any plans to support physics calculations in GPU? Or is it done already?}
