...
/* -*- mode: c++; c-basic-offset: 4; indent-tabs-mode: nil -*-
   ...
   $Id: RCCMonitor.cpp 246 2011-02-18 18:44:14Z sgvandijk $
...
RCCMonitor::RCCMonitor() : oxygen::MonitorSystem()
{
    mFullState = true;
}

RCCMonitor::~RCCMonitor()
{
}

void RCCMonitor::ClearNodeCache()
{
    mNodeCache.clear();
}

void RCCMonitor::UpdateCached()
{
    MonitorSystem::UpdateCached();
    ClearNodeCache();
}

void RCCMonitor::OnLink()
{
    ...
    if (mSceneServer.get() == 0)
        {
            GetLog()->Error()
                << "(RCCMonitor) ERROR: SceneServer not found\n";
        }
...
void RCCMonitor::ParseMonitorMessage(const std::string& data)
{
...
string RCCMonitor::GetMonitorInfo(const PredicateList& pList)
{
    stringstream ss;
    mFullState = false;
...
string RCCMonitor::GetMonitorHeaderInfo(const PredicateList& pList)
{
    stringstream ss;
    mFullState = true;
    ClearNodeCache();
...
void RCCMonitor::DescribeCustomPredicates(stringstream& ss,const PredicateList& pList)
{
    ss << "(";
...
void RCCMonitor::DescribeBaseNode(stringstream& ss)
{
    if (mFullState)
        {
            ss << "(nd BN";
...
void RCCMonitor::DescribeLight(stringstream& ss, boost::shared_ptr<Light> light)
{
    if (! mFullState)
        {
...
void RCCMonitor::DescribeTransform(stringstream& ss, NodeCache& entry, boost::shared_ptr<Transform> transform)
{
    if (mFullState)
        {
            ss << "(nd TRF";
...
void RCCMonitor::DescribeMesh(stringstream& ss, boost::shared_ptr<StaticMesh> mesh)
{
...
void RCCMonitor::DescribeAgentState(stringstream& ss, NodeCache& entry, boost::shared_ptr<AgentState> agentstate)
{
    if (mFullState)
        {
            ss << "(nd AS ";
    ...
    int t = agentstate->GetTeamIndex();
    int u = agentstate->GetUniformNumber();
    
    if (mFullState || entry.ints["team"] != t || entry.ints["unum"] != u)
        {
            ss << "(team " << (t == TI_LEFT ? "Left" : t == TI_RIGHT ? "Right" : "None") << ")"
               << "(unum " << u << ")";
            entry.ints["team"] = t;
            entry.ints["unum"] = u;
        }
    ...
    string msg;
    agentstate->GetSelfMessage(msg, false);
        
    if (msg != "" && agentstate->GetSelfMessageTimeStamp() != entry.floats["say"])
        {
            ss << "(say " << msg << ")";
            entry.floats["say"] = agentstate->GetSelfMessageTimeStamp();
        }
}

RCCMonitor::NodeCache* RCCMonitor::LookupNode(boost::shared_ptr<BaseNode> node)
{
    if (node.get() == 0)
        {
            assert(false);
    ...
    boost::shared_ptr<AgentState> agentstate
        = shared_dynamic_cast<AgentState>(node);
    if (agentstate.get() != 0)
        {
            mNodeCache[node] = NodeCache(NT_AGENTSTATE);
            return &(mNodeCache[node]);
        }
...
bool RCCMonitor::DescribeNode(stringstream& ss, boost::shared_ptr<BaseNode> node)
{
    ...
    switch (entry->type)
        ...
        case NT_AGENTSTATE:
            DescribeAgentState
                (ss, (*entry), shared_static_cast<AgentState>(node));
            return true;
        }
}

void RCCMonitor::DescribeActiveScene(stringstream& ss)
{
    if (mSceneServer.get() == 0)
        {
            return;
...
void RCCMonitor::DescribeScene(stringstream& ss, boost::shared_ptr<BaseNode> node)
{
